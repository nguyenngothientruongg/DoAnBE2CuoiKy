...
<main class="main-container bg">
    ...
    <header>
        <h4 class="section-title text-white Top10ShowInVN" style="font-family: Cursive;">Người mới ư? Những thứ này dành
            cho bạn</h4>
    </header>
    ...
    <section class="row" style="overflow-y: scroll;">
        <div class="col-12">
            <div class="d-flex flex-row">
                <div class="d-flex flex-row">
                    @foreach($movies->slice(8, 8) as $movie)
                        <div class="col-sm-6 col-md-4 col-lg-3 px-1">
                            <a href="{{ route('user.showMovie', ['id' => $movie->id, 'tuoi' => \Carbon\Carbon::parse(Auth::user()->date)->age]) }}"
                                style="text-decoration: none;">
                                <div class="card bg text-white">
                                    @if($movie->age_limit)
                                        <span class="badge bg-danger"
                                            style="position: absolute; top: 10px; right: 10px;">18+</span>
                                    @endif
                                    <img src="{{ $movie->images }}" alt="{{ $movie->movie_name }}"
                                        class="card-img-top mx-auto d-block"
                                        style="width: 200px; height: 150px; object-fit: cover;">
    ...
    <header>
        <h4 class="section-title text-white Top10ShowInVN" style="font-family: Cursive;">Phổi bạn không ổn? Để tôi cho
            bạn vài Drama để hít nhé!</h4>
    </header>
    <section class="row" style="overflow-y: scroll;">
        <div class="col-12">
            <div class="d-flex flex-row">
                @foreach($movies->slice(16, 7) as $movie)
                    <div class="col-sm-6 col-md-4 col-lg-3 px-1">
                        <a href="{{ route('user.showMovie', ['id' => $movie->id, 'tuoi' => \Carbon\Carbon::parse(Auth::user()->date)->age]) }}"
                            style="text-decoration: none;">
                            <div class="card bg text-white">
                                @if($movie->age_limit)
                                    <span class="badge bg-danger" style="position: absolute; top: 10px; right: 10px;">18+</span>
                                @endif
                                <img src="{{ $movie->images }}" alt="{{ $movie->movie_name }}"
                                    class="card-img-top mx-auto d-block"
                                    style="width: 200px; height: 150px; object-fit: cover;">
    ...
    <header>
        <h4 class="section-title text-white Top10ShowInVN" style="font-family: Cursive;">Chưa đủ tuổi? Đừng lo, chúng
            tôi đáp ứng hết</h4>
    </header>
    <section class="row" style="overflow-y: scroll;">
        <div class="col-12">
            <div class="d-flex flex-row">
                @foreach($movies->slice(23, 7) as $movie)
                    <div class="col-sm-6 col-md-4 col-lg-3 px-1">
                        <a href="{{ route('user.showMovie', ['id' => $movie->id, 'tuoi' => \Carbon\Carbon::parse(Auth::user()->date)->age]) }}"
                            style="text-decoration: none;">
                            <div class="card bg text-white">
